...
vector< vector <float> > initialize_beliefs(vector< vector <char> > grid) {
	...
	float beleif_per_cell=1.0/((float)area);
        vector <float> newGrid_row;
        
        for (int i=0;i<height;i++){
         newGrid_row.clear();
          for (int j=0;j<width;j++){
           newGrid_row.push_back(beleif_per_cell);
	...
	float p_miss) 
{
         ...
         int row=grid.size();
         int col=grid[0].size();
        vector < vector <float> > newGrid(row,vector<float>(col,0));
        for (int i=0;i<row;i++){
         for (int j=0;j<col;j++){
              char actual=grid[i][j];
               if(color==actual){p=p_hit;}
               if(color!=actual){p=p_miss;}
                newGrid[i][j]=p*beliefs[i][j];
           }
        }
	...
	float blurring){ 
        ...
        for (int i=0;i<height;i++){
         for (int j=0;j<width;j++){
              ...
              float value=beliefs[i][j];
              newGrid[new_i][new_j]=value;
              } 
